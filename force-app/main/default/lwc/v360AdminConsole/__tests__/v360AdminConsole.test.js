...
describe('c-v360-admin-console', () => {
    ...
    it('picks a new icon for the open card from the icon picker and saves it', async () => {
        ...
        expect(element.shadowRoot.querySelector('[data-id="prop-icon-trigger"]').title).toBe('standard:contact');
    ...
    it('resets the icon draft when switching cards, so the picker preselects the newly opened card', async () => {
        ...
        expect(element.shadowRoot.querySelector('[data-id="prop-icon-trigger"]').title).toBe('standard:account');
    });
